...
from official.projects.videoglue.configs import (
    spatiotemporal_action_localization as cfg,
)
from official.projects.videoglue.modeling.backbones import (
    vit_3d,
...
class VideoActionTransformerModel(tf_keras.Model):
        ...
        **kwargs
        ...
        if not input_specs:
            input_specs = {
                ...
                "instances_position": tf_keras.layers.InputSpec(shape=[None, None, 4]),
            }
    ...
    def _build_model(
        ...
        inputs = {k: tf_keras.Input(shape=v.shape[1:]) for k, v in input_specs.items()}
...
def build_video_action_transformer_model(
    ...
    if model_config.norm_activation.use_sync_bn != model_config.head.use_sync_bn:
